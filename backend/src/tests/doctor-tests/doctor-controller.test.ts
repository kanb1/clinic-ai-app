...
describe("Doctor Controller", () => {
  ...
  it("should get AI chat session for an appointment", async () => {
    ...
    await ChatSessionModel.create({
      ...
      messages: [
        ...
        { user: "Jeg har ondt i hovedet", ai: "Det kan skyldes flere ting..." },
      ],
      patient_id: patient._id,
    });
  ...
  it("should seed a journal entry using test route", async () => {
    const { token, clinicId } = await createDoctorWithToken();
    const patient = await UserModel.create({
      name: "Seed Entry Patient",
      email: "seed@test.com",
      password_hash: "test",
      role: "patient",
      clinic_id: clinicId,
    ...
    const res = await request(app)
      ...
      .send({
        ...
        notes: "Note from seed",
        created_by_ai: true,
      });

    expect(res.status).toBe(201);
    expect(res.body.entry.notes).toBe("Note from seed");
  });

  it("should seed test result using test route", async () => {
    const { token, clinicId } = await createDoctorWithToken();
    const patient = await UserModel.create({
      name: "TestResult Patient",
      email: "tr@test.com",
      password_hash: "test",
      role: "patient",
      clinic_id: clinicId,
    ...
    expect(res.status).toBe(201);
    expect(res.body.newResult.test_name).toBe("Covid-19");
  });

  it("should create a test journal entry (SEED)", async () => {
    const { token, clinicId, doctor } = await createDoctorWithToken();
    const patient = await UserModel.create({
      name: "Seed Patient",
    ...
    expect(res.status).toBe(201);
    expect(res.body.entry.notes).toBe("Seed note");
  });

  it("should create a test result (SEED)", async () => {
    const { token, clinicId } = await createDoctorWithToken();
    const patient = await UserModel.create({
      name: "Test Result Seed",
      email: `trs-${Date.now()}@test.com`,
      phone: "12345678",
      password_hash: "1234",
      role: "patient",
      clinic_id: clinicId,
    ...
    expect(res.status).toBe(201);
    expect(res.body.newResult.result).toBe("Negativ");
  });
