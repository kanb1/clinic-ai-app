// supertest -> simulerer HTTP-req mod min express API i testmiljø
...
describe("Admin Staff Controller", () => {
  describe("Get staff", () => {
    it("should get all doctors in clinic", async () => {
      // helper -> opretter admin, klinik, jwt-token
      const { admin, token, clinicId } = await createAdminWithClinicAndToken();

      await UserModel.create({
        name: "Dr. Test",
        //Date.now() -> gør email/telefon unikke
      ...
      // simulerer request til endpoints og sætter token i headers
      const res = await request(app)
        .get("/api/admin/staff/doctors-list")
        .set("Authorization", `Bearer ${token}`);

      // Forventninger (assertions)
      expect(res.status).toBe(200);
      // listen over læger -> skal indeholde min. 1 læge
  ...
  describe("Update staff", () => {
    it("should update a doctor", async () => {
      ...
      const res = await request(app)
        // .send -> opdaterer kun name, resten sendes uændres
